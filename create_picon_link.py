...
# Source originario https://github.com/OpenPLi/enigma2/blob/master/tools/create_picon_links.py
# Modificado Lucifer error en creacion service reference canales HD en conversion
...
#	ref = [int(x, 0x10) for x in f[0][:-1].split(':')]
	p = f[0][:-1].split(':')
	ref = [int(p[0], 0x10), int(p[1], 0x10), int(p[2], 0x10), int(p[3], 0x10), int(p[4])]
	name = f[1][:-1]
